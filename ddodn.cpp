// ddodn deck donkey odrive new
// derived mostly from ddod code started in Sept 2022
// copy right Andy Laberge September 2022
// this code is free to use under the Qt open source license LGPLv3 and GPLv3
// copies of which should be in the same folder as this code.

// this code can be written on Ubuntu 20.04 running on Intel PC and/or
// Ubuntu 18.04 on Jetson Nano running ARM processor...the machines are running
// QtCreator natively...there are compiling quirks especially on the Jetson Nano
// I will tried to address those issues in another file which when written will
// be noted here...till that time you are on your own
//
// note 13 Sep 2022
// ddodn is going to be borrowing code from ddod and w4od at least for a while
// the end code will be running on Jetson Nano which is controlling an Odrive 3.6-56v
// BLDC motor controller which is driving two RoboWheels(170mm wheel size) using
// 3200 resolution optical encoder (non-indexed)
//
...
/*
void ddodn::on_Received_odrv_2_Data()
{
   // QByteArray read_ba;
...
void ddodn::on_Received_odrv_3_Data()
{
   // QByteArray read_ba;
...
void ddodn::on_forward_pushButton_pressed()
{
...
void ddodn::on_left_pushButton_pressed()
{
    avoid_flag=false;
    left_pressed();
}

void ddodn::left_pressed()
{
...
void ddodn::on_right_pushButton_pressed()
{
    avoid_flag=false;
    right_pressed();
}
void ddodn::right_pressed()
{
...
void ddodn::on_forward_pushButton_released()
{
    avoid_flag=false;
    forward_released();
}

void ddodn::forward_released()
{
...
void ddodn::on_reverse_pushButton_pressed()
{
    avoid_flag=false;  //
    str_velocity=QString::number(velocity_bu);
    // check to see if front wheel drive is engaged
...
void ddodn::on_reverse_pushButton_released()
{
    avoid_flag=false;
    reverse_released();
}

void ddodn::reverse_released()
{
    on_avoidance_checkBox_clicked(true);
    str_velocity = "0.00";
    // check to see if front wheel drive is engaged
...
void ddodn::on_left_pushButton_released()
{
    avoid_flag=false;
    left_released();
}

void ddodn::left_released()
{
    str_velocity = "0.00";
    // check to see if front wheel drive is engaged
    if(front_wd_flag==true)
...
void ddodn::on_right_pushButton_released()
{
    avoid_flag=false;
    right_released();
}

void ddodn::right_released()
{
    str_velocity = "0.00";
    // check to see if front wheel drive is engaged
    if(front_wd_flag==true)
...
void ddodn::on_pivot_starboard_pushButton_pressed()
{
...
void ddodn::on_pivot_port_pushButton_pressed()
{
    str_pvt_spd=QString::number(pvt_spd);
  /*  // check to see if front wheel drive is engaged
    if(front_wd_flag==true)
...
void ddodn::on_pivot_port_pushButton_released()
{
    str_pvt_spd = "0.00";
    // check to see if front wheel drive is engaged
    if(front_wd_flag==true)
...
void ddodn::on_pivot_starboard_pushButton_released()
{
    str_pvt_spd = "0.00";
    // check to see if front wheel drive is engaged
    if(front_wd_flag==true)
...
void ddodn::on_show_volt_pushButton_clicked()
{
    if(odrv_3->isOpen()==true){
...
void ddodn::on_closed_loop_pushButton_clicked()
{
 /*
...
void ddodn::on_idle_pushButton_clicked()
{
...
void ddodn::on_cal_encoder_pushButton_clicked()
{
...
void ddodn::on_turn_ratio_doubleSpinBox_valueChanged(double arg1)
{
    steer_divisor = arg1;
}


void ddodn::on_speed_doubleSpinBox_valueChanged(double arg1)
{
    velocity_bu = arg1;
    if(ui->speed_constant_checkBox->isChecked()==true)
        on_forward_pushButton_pressed();
...
void ddodn::on_pivot_spd_doubleSpinBox_valueChanged(double arg1)
{
    pvt_spd =arg1;

}

void ddodn::on_checkBox_stateChanged(int arg1)
{
    if(arg1!=0)
    {
        front_wd_flag = true;
...
void ddodn::on_sys_info_pushButton_clicked()
{
        first_info();
        second_info();
}
...
void ddodn::on_speed_constant_checkBox_clicked(bool checked)
{
    if(checked==false)
        on_forward_pushButton_released();
...
void ddodn::on_avoidance_checkBox_clicked(bool checked)
{
    if(checked==true)
        avoid_flag=true;
...
void ddodn::on_unjam_pushButton_clicked()
{
...
void ddodn::timed_reverse_release()
{
...
void ddodn::on_reboot_odrv_pushButton_clicked()
{
